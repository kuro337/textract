...
#pragma region FILE_IO                    /* File IO Declarations */
    ...
    auto createDirectories(const llvm::Twine &path) -> bool;
...
#pragma region OPENCV_UTILS               /* OpenCV Declarations */
    ...
    std::string getTextOCROpenCV(const std::vector<unsigned char> &file_content, const std::string &lang);

    std::string extractTextFromImageBytes(const std::vector<unsigned char> &file_content, const std::string &lang);
...
#pragma region ASYNC_LOGGER      /* Non Blocking Asynchronous Logger with ANSI Escaping */
    ...
    class AsyncLogger {
      public:
        ...
        class LogStream {
          public:
            LogStream(AsyncLogger &logger, bool immediateFlush = true): logger(logger), immediateFlush(immediateFlush) {
            }
...
#pragma region TESSERACT_OPENMP          /* Tesseract Implementation for Thread Local Tesseracts'  */
    ...
    struct TesseractOCR {
        ...
        auto operator->() const -> tesseract::TessBaseAPI * {
            return ocrPtr;
        }
    };
...
#pragma region imgstr_core      /* Core Class for Image Processing and Text Extraction */
    ...
    struct Image {
        ...
        Image(std::string img_hash, std::string path, const std::string &text_content, size_t image_size = 0):
            mutex(nullptr),
            path(std::move(path)),
    ...
    class ImgProcessor {
      private:
        ...
        auto processImageFile(const std::string &file) -> std::optional<std::reference_wrapper<const Image>> {
#ifdef _DEBUGAPP
        ...
        auto getFromCacheIfExists(const std::string &img_sha) -> std::optional<std::reference_wrapper<const Image>> {
        ...
        void printFileProcessingFailure(const std::string &file, const std::string &err_msg) {
            logger->log() << "Failed to Extract Text from Image file: " << file << ". Error: " << err_msg << '\n';
        }
        ...
        void destructionLog() {
            logger->log() << LIGHT_GREY << "Destructor called - freeing " << BRIGHT_WHITE
                ...
                BRIGHT_WHITE << "Total Images Processed :: " << BOLD_WHITE << processedImagesCount << END << '\n';
        }
      ...
      public:
        ...
        auto getImageText(const std::string &file_path, ISOLang lang = ISOLang::en) -> std::optional<std::string> {
        ...
        void simpleProcessDir(const std::string &directory, const std::string &output_path = "") {
            ...
            auto                     filePaths = getFilePaths(directory);
...
#pragma omp parallel for
                for (const auto &imagePath: imageFiles) {
                    ...
                    auto out_path     = createQualifiedFilePath(imagePath, output_path);
        ...
        void convertImagesToTextFilesParallel(const std::string &output_dir = "", ISOLang lang = ISOLang::en) {
...
#pragma omp parallel for
        ...
        void convertImagesToTextFiles(const std::string &output_dir = "", ISOLang lang = ISOLang::en) {
            createDirIfNotExists(output_dir);
                if (queued.empty()) {
                    filesAlreadyProcessedLog();
                    return;
...
#pragma omp parallel for
        ...
        void setImageMode(ImgMode img_mode) {
        ...
        std::vector<std::vector<size_t>> dp(m + 1, std::vector<size_t>(n + 1));
            ...
            for (size_t i = 1; i <= m; i++) {
                    for (size_t j = 1; j <= n; j++) {
                        ...
                        dp[i][j] = std::min(dp[i - 1][j - 1] + equal, std::min(dp[i - 1][j] + 1, dp[i][j - 1] + 1));
                    }
            }
...
#pragma region OPENCV_IMPL                /* OpenCV Image Processing Implementations */
    ...
    static const std::unordered_set<std::string> validExtensions = {".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tif"};

    inline auto isImageFile(const llvm::StringRef &path) -> bool {
        return validExtensions.find(llvm::sys::path::extension(path).str()) != validExtensions.end();
    }
    ...
    inline void createPDF(const std::string &input_path, const std::string &output_path, const char *datapath) {
        bool textonly = false;
...
#include <opencv2/imgproc.hpp>
    ...
    inline std::string getTextOCROpenCV(const std::vector<uchar> &file_content, const std::string &lang) {
    ...
    inline std::string extractTextFromImageFile(const std::string &file_path, const std::string &lang) {
...
#pragma region FILE_IO_IMPL               /* STL File I/O Implementations */
    ...
    inline auto getFilePaths(const llvm::Twine &directoryPath) -> std::vector<std::string> {
        std::error_code                   ERR;
        ...
        std::vector<std::string> filePaths;
            ...
            for (; dirIt != dirEnd && !ERR; dirIt.increment(ERR)) {
                    if (ERR) {
                        llvm::errs() << "Error reading directory: " << ERR.message() << '\n';
                        break;
                }
    ...
    inline auto createDirectories(const llvm::Twine &path) -> bool {
            if (auto err = llvm::sys::fs::create_directories(path); err) {
                serr << llvm::formatv("Error creating directory {0} : {1}\n", path, err.message());
                return false;
        }
        return true;
    }
...
#pragma region SYSTEM_IMPL                /* System Environment Util Implementations */
    ...
    auto inline fileExists(const std::string &path) -> bool {
    ...
    inline auto getStartTime() -> time_point {
