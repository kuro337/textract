...
inline auto
isoToTesseractLang(ISOLang isoLang) -> std::string {
    switch (isoLang) {
    case ISOLang::en:
        return "eng";
    case ISOLang::es:
...
auto getTextOCR(const std::vector<unsigned char> &file_content,
                const std::string &lang, ImgMode img_mode) -> std::string;
  ...
  public:
    ...
    class LogStream {
      public:
        ...
        template <typename T> auto operator<<(const T &msg) -> LogStream & {
            stream << msg;
            return *this;
        }
...
struct Image {
    ...
    Image(std::string img_hash, std::string path,
          const std::string &text_content, size_t image_size = 0)
        : image_sha256(std::move(img_hash)), path(std::move(path)),
          image_size(image_size), text_content(text_content),
          text_size(text_content.size()), mutex(nullptr) {
...
class ImgProcessor {
  ...
  public:
    ...
    ImgProcessor(size_t capacity = 1000)
        ...
        : num_cores(CORES::single), logger(std::make_unique<AsyncLogger>()),
...
#pragma omp parallel for
        for (const auto &filePath : filePaths) {
            START_TIMING();
            auto file_content = readBytesFromFile(filePath);
            auto img_text = getTextOCRNoClear(file_content);
            auto out_path = createQualifiedFilePath(filePath, output_path);
...
inline auto
levenshteinScore(std::string a, std::string b) -> size_t {
    ...
    for (size_t i = 1; i <= m; i++) {
        for (size_t j = 1; j <= n; j++) {
            ...
            dp[i][j] = std::min(dp[i - 1][j - 1] + equal,
...
inline auto
computeSHA256(const std::vector<unsigned char> &data) -> std::string {
    ...
    //    std::vector<char> hash(EVP_MD_size(EVP_sha256()));
...
inline auto
computeSHA256(const std::string &filePath) -> std::string {
...
getTextOCR(const std::vector<unsigned char> &file_content,
    ...
    Pix *image = pixReadMem(static_cast<const l_uint8 *>(file_content.data()),
                            file_content.size());
    if (image == nullptr) {
...
inline auto
getTextImgFile(const std::string &file_path,
...
inline auto
createQualifiedFilePath(const std::string &fileName,
                        const std::string &directory,
...
inline auto
getCurrentTimestamp() -> std::string {
...
inline auto
getDuration(const time_point &startTime) -> double {
